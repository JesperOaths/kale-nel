(function(){
  if (window.GEJAST_ACCOUNT_RUNTIME && window.GEJAST_ACCOUNT_RUNTIME.VERSION === 'v679') return;
  ...
  const LOGIN_CACHE_PREFIX = 'gejast_login_active_names_v679_';
  ...
  async function getLoginNames(){
    ...
    const attempts = [
      { name:'get_login_active_names_v679', body:{site_scope_input:scope()}, options:{timeoutMs:2200} },
  ...
  function activePlayerMeta(extra){
    return visitorMeta(Object.assign({
      metadata_policy: 'new_active_users_v679',
  ...
  async function storeActivePlayerMetadata(payload){
    try {
      return await rpc('store_active_player_metadata_v679', Object.assign({
  ...
  async function adminAudit(){ return await rpcFirst([{name:'admin_get_active_player_metadata_audit_v679', body:{admin_session_token_input:adminToken(),site_scope_input:scope()}},{name:'admin_get_account_runtime_audit_v671', body:{admin_session_token:adminToken(),site_scope_input:scope()}}]); }
  ...
  async function adminDiagnoseLogin(name){ return await rpcFirst([{name:'diagnose_login_name_v679', body:{player_name_input:name,site_scope_input:scope()}},{name:'diagnose_login_name_v678', body:{player_name_input:name,site_scope_input:scope()}}]); }
  async function adminRepairLogin(name,pin){ return await rpcFirst([{name:'admin_reset_login_player_pin_v679', body:{admin_session_token_input:adminToken(),player_name_input:name,new_pin_input:pin,site_scope_input:scope(),admin_meta_input:activePlayerMeta({event:'admin_reset_login_player_pin', admin_page:'admin_account_runtime.html'})}},{name:'admin_reset_login_player_pin_v678', body:{admin_session_token_input:adminToken(),player_name_input:name,new_pin_input:pin,site_scope_input:scope()}}]); }
  ...
  async function adminApprove(id){ return await rpcFirst([{name:'admin_approve_account_claim_v679', body:{admin_session_token_input:adminToken(),claim_id_input:id,site_scope_input:scope(),admin_meta_input:activePlayerMeta({event:'admin_approve_claim', admin_page:'admin_account_runtime.html'})}},{name:'admin_approve_account_claim_v671', body:{admin_session_token_input:adminToken(),claim_id_input:id,site_scope_input:scope()}}]); }

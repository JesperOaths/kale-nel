(function(){
  if (window.GEJAST_ACCOUNT_RUNTIME && window.GEJAST_ACCOUNT_RUNTIME.VERSION === 'v685') return;
  ...
  const LOGIN_CACHE_PREFIX = 'gejast_login_active_names_v685_';
  ...
  async function getLoginNames(){
    ...
    // One fast authoritative call first: legacy public.players with active=true + pin_hash, plus v685 activated accounts.
    // Do not use requestable/scope-only/profile names; those caused the polluted dropdown.
    const attempts = [
      { name:'get_login_active_names_v685', body:{site_scope_input:scope()}, options:{timeoutMs:2200} },
      { name:'get_login_active_names_v685', body:{site_scope_input:scope()}, options:{timeoutMs:2200} },
      { name:'get_login_active_names_v685', body:{site_scope_input:scope()}, options:{timeoutMs:2200} },
      { name:'get_login_active_names_v685', body:{site_scope_input:scope()}, options:{timeoutMs:2200} },
      { name:'get_login_active_names_v685', body:{site_scope_input:scope()}, options:{timeoutMs:2200} }
  ...
  async function getRequestableNames(){
    const data = await rpcFirst([
      {name:'get_requestable_names_v685', body:{site_scope_input:scope()}},
  ...
  async function requestClaim(input){
    ...
    return await rpcFirst([
      {name:'account_request_claim_v685', body:{desired_name_input:input.desiredName, requester_email_input:input.email, requester_note_input:input.note||'', site_scope_input:scope(), requester_meta_input:input.meta||meta, active_player_meta_input:meta}},
      {name:'account_request_claim_v685', body:{desired_name_input:input.desiredName, requester_email_input:input.email, requester_note_input:input.note||'', site_scope_input:scope(), requester_meta_input:input.meta||meta, active_player_meta_input:meta}},
      {name:'account_request_claim_v685', body:{desired_name_input:input.desiredName, requester_email_input:input.email, requester_note_input:input.note||'', site_scope_input:scope(), requester_meta_input:input.meta||meta}},
  ...
  async function getActivationContext(token){ return await rpcFirst([{name:'account_get_activation_context_v685', body:{activation_token_input:token}},{name:'get_activation_link_context', body:{token}},{name:'get_activation_link_context', body:{activation_token:token}}]); }
  async function activateAccount(token,pin){
    ...
    const out = await rpcFirst([
      {name:'account_activate_v685', body:{activation_token_input:token, new_pin_input:pin, activation_meta_input:meta}},
      {name:'account_activate_v685', body:{activation_token_input:token, new_pin_input:pin, activation_meta_input:meta}},
      {name:'account_activate_v685', body:{activation_token_input:token, new_pin_input:pin}},
  ...
  async function login(input){
    ...
    const data = await rpcFirst([
      {name:'account_login_bridge_v685', body:{display_name_input:name, pin_input:pin, site_scope_input:scope()}, options:{timeoutMs:3200}},
      {name:'account_login_bridge_v685', body:{display_name_input:name, pin_input:pin, site_scope_input:scope()}, options:{timeoutMs:3200}},
      {name:'account_login_bridge_v685', body:{display_name_input:name, pin_input:pin, site_scope_input:scope()}, options:{timeoutMs:3200}},
      {name:'account_login_v685', body:{display_name_input:name, pin_input:pin, site_scope_input:scope()}, options:{timeoutMs:3200}},
  ...
  async function getPublicState(){ const token=playerToken(); if(!token) return null; try{ return await rpcFirst([{name:'get_public_state', body:{session_token:token}, options:{timeoutMs:1200}},{name:'account_public_state_v685', body:{session_token_input:token}, options:{timeoutMs:1200}}]); }catch(_){ return null; } }
  ...
  function activePlayerMeta(extra){
    return visitorMeta(Object.assign({
      metadata_policy: 'new_active_users_v685_plus_scope_override_v685',
  ...
  async function storeActivePlayerMetadata(payload){
    try {
      return await rpc('store_active_player_metadata_v685', Object.assign({
  ...
  async function adminAudit(){ return await rpcFirst([{name:'admin_get_active_player_metadata_audit_v685', body:{admin_session_token_input:adminToken(),site_scope_input:scope()}},{name:'admin_get_active_player_metadata_audit_v685', body:{admin_session_token_input:adminToken(),site_scope_input:scope()}},{name:'admin_get_account_runtime_audit_v685', body:{admin_session_token:adminToken(),site_scope_input:scope()}}]); }
  ...
  async function adminDiagnoseLogin(name){ return await rpcFirst([{name:'diagnose_login_name_v685', body:{player_name_input:name,site_scope_input:scope()}},{name:'diagnose_login_name_v685', body:{player_name_input:name,site_scope_input:scope()}},{name:'diagnose_login_name_v685', body:{player_name_input:name,site_scope_input:scope()}}]); }
  async function adminRepairLogin(name,pin){ return await rpcFirst([{name:'admin_reset_login_player_pin_v685', body:{admin_session_token_input:adminToken(),player_name_input:name,new_pin_input:pin,site_scope_input:scope(),admin_meta_input:activePlayerMeta({event:'admin_reset_login_player_pin', admin_page:'admin_account_runtime.html'})}},{name:'admin_reset_login_player_pin_v685', body:{admin_session_token_input:adminToken(),player_name_input:name,new_pin_input:pin,site_scope_input:scope(),admin_meta_input:activePlayerMeta({event:'admin_reset_login_player_pin', admin_page:'admin_account_runtime.html'})}},{name:'admin_reset_login_player_pin_v685', body:{admin_session_token_input:adminToken(),player_name_input:name,new_pin_input:pin,site_scope_input:scope()}}]); }
  async function adminAddName(name){ return await rpc('admin_add_requestable_name_v685',{admin_session_token_input:adminToken(),display_name_input:name,site_scope_input:scope()}); }
  async function adminApprove(id){ return await rpcFirst([{name:'admin_approve_account_claim_v685', body:{admin_session_token_input:adminToken(),claim_id_input:id,site_scope_input:scope(),admin_meta_input:activePlayerMeta({event:'admin_approve_claim', admin_page:'admin_account_runtime.html'})}},{name:'admin_approve_account_claim_v685', body:{admin_session_token_input:adminToken(),claim_id_input:id,site_scope_input:scope(),admin_meta_input:activePlayerMeta({event:'admin_approve_claim', admin_page:'admin_account_runtime.html'})}},{name:'admin_approve_account_claim_v685', body:{admin_session_token_input:adminToken(),claim_id_input:id,site_scope_input:scope()}}]); }
  async function adminReject(id,reason){ return await rpc('admin_reject_account_claim_v685',{admin_session_token_input:adminToken(),claim_id_input:id,reject_reason_input:reason||'',site_scope_input:scope()}); }

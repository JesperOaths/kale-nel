(function(){
  ...
  async function rpc(name, payload){ if(!cfg.SUPABASE_URL) throw new Error('Supabase URL ontbreekt.'); const raw=await fetch(`${cfg.SUPABASE_URL}/rest/v1/rpc/${name}`,{method:'POST',mode:'cors',cache:'no-store',headers:headers(),body:JSON.stringify(payload||{})}).then(parse); return raw && raw[name] !== undefined ? raw[name] : raw; }
  async function rpcFirst(calls){ let last=null; for(const c of calls){ try{ return await rpc(c.name,c.body||{}); }catch(err){ last=err; } } throw last || new Error('RPC mislukt.'); }
  ...
  function adminToken(){ try{ if(window.GEJAST_ADMIN_SESSION&&window.GEJAST_ADMIN_SESSION.getToken) return String(window.GEJAST_ADMIN_SESSION.getToken()||'').trim(); }catch(_){} for(const k of ADMIN_KEYS){ const v=localStorage.getItem(k)||sessionStorage.getItem(k); if(v) return String(v).trim(); } return ''; }
  function normalizeNames(rows){ const out=[]; const seen=new Set(); (Array.isArray(rows)?rows:[]).forEach((row)=>{ const name=String(typeof row==='string'?row:(row.display_name||row.name||row.desired_name||row.player_name||'')).replace(/\s+/g,' ').trim(); const key=name.toLowerCase(); if(name&&!seen.has(key)){ seen.add(key); out.push(name); } }); return out.sort((a,b)=>a.localeCompare(b,'nl')); }
  function friendly(err){ const msg=String(err&&err.message?err.message:err||'Er ging iets mis'); if(msg==='Failed to fetch') return 'Netwerk/CORS-fout bij contact met Supabase.'; if(/timeout/i.test(msg)) return 'Supabase reageerde te traag. Probeer opnieuw.'; if(/could not find|schema cache|function/i.test(msg)) return 'Backend-RPC ontbreekt of cache is nog niet ververst. Draai de SQL en refresh Supabase schema cache.'; return msg; }
  ...
  async function requestClaim(input){
    return await rpcFirst([
      {name:'account_request_claim_v671', body:{desired_name_input:input.desiredName, requester_email_input:input.email, requester_note_input:input.note||'', site_scope_input:scope(), requester_meta_input:input.meta||{}}},
      {name:'request_claim_action', body:{desired_name:input.desiredName, requester_email:input.email, requester_note:input.note||'', requester_meta:input.meta||{}, site_scope_input:scope()}}
    ]);
  }
  async function getActivationContext(token){
    return await rpcFirst([
      {name:'account_get_activation_context_v671', body:{activation_token_input:token}},
      {name:'get_activation_link_context', body:{token}},
      {name:'get_activation_link_context', body:{activation_token:token}}
    ]);
  }
  ...
  async function login(input){
    ...
    const data = await rpcFirst([
      ...
      {name:'account_login_v671', body:{display_name_input:name, pin_input:pin, site_scope_input:scope()}}
    ]);
    if(data?.session_token) setPlayerToken(data.session_token);
    return data;
  }
  async function getPublicState(){
    const token=playerToken(); if(!token) return null;
    try{ return await rpcFirst([{name:'get_public_state', body:{session_token:token}}, {name:'account_public_state_v671', body:{session_token_input:token}}]); }catch(_){ return null; }
  }
  ...
  async function bootRequestPage(){
    const form=$('requestForm'), sel=$('requestNameSelect'); if(!form) return;
    try{ fillSelect(sel, await getRequestableNames()); }catch(err){ setStatus('status',friendly(err),'warn'); }
    form.addEventListener('submit', async(ev)=>{ ev.preventDefault(); const desiredName=String(sel.value||'').trim(); const email=String(($('requestEmailInput')||{}).value||'').trim().toLowerCase(); const note=String(($('requestNoteInput')||{}).value||'').trim(); if(!desiredName) return setStatus('status','Kies eerst een naam.','warn'); if(!emailOk(email)) return setStatus('status','Vul een geldig e-mailadres in.','warn'); try{ setBusy(form,true); setStatus('status','Aanvraag versturen...'); const out=await requestClaim({desiredName,email,note,meta:visitorMeta()}); setStatus('status',out?.message||'Aanvraag verstuurd. Na goedkeuring ontvang je een activatielink.','ok'); form.reset(); try{ fillSelect(sel, await getRequestableNames()); }catch(_){} }catch(err){ setStatus('status',friendly(err),'warn'); }finally{ setBusy(form,false); } });
  }
  async function bootActivatePage(){
    const form=$('activateForm'); if(!form) return; const token=new URLSearchParams(location.search).get('token')||new URLSearchParams(location.search).get('activation_token')||'';
    if(!token) setStatus('status','Deze activatielink mist een token.','warn');
    try{ const ctx=await getActivationContext(token); if($('approvedName')) $('approvedName').textContent=ctx?.display_name||ctx?.player_name||ctx?.desired_name||'Onbekend'; if($('approvedEmail')) $('approvedEmail').textContent=ctx?.requester_email||ctx?.email||'Onbekend'; }catch(err){ setStatus('status',friendly(err),'warn'); }
    form.addEventListener('submit', async(ev)=>{ ev.preventDefault(); const p=String(($('pinInput')||{}).value||'').replace(/\D/g,'').slice(0,4); const p2=String(($('pinConfirmInput')||{}).value||'').replace(/\D/g,'').slice(0,4); if(!/^\d{4}$/.test(p)) return setStatus('status','Kies een pincode van precies 4 cijfers.','warn'); if(p!==p2) return setStatus('status','De twee pincodes komen niet overeen.','warn'); try{ setBusy(form,true); setStatus('status','Account activeren...'); const out=await activateAccount(token,p); if(out?.session_token) setPlayerToken(out.session_token); setStatus('status','Account geactiveerd. Je gaat naar de homepage.','ok'); setTimeout(()=>location.href='./index.html',700); }catch(err){ setStatus('status',friendly(err),'warn'); }finally{ setBusy(form,false); } });

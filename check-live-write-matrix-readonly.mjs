...
await row('BRIDGE-01 invalid session save rejection', ['boerenbridge_matches'], async () => {
  const out = await rpc('save_boerenbridge_match', { session_token: invalidPlayerToken, client_match_id: `${testLabel}_BRIDGE`, rules_version: 'matrix', app_version: 'v772', match_payload: { source: testLabel, players: ['Test A', 'Test B'], totals: [] } });
  return rejectionSummary(out);
});

...
Files in the flat patch bundle:
- VERSION
...
- Four passes without a current bid reshuffle/redeal and keep bidding alive.
- Roem tally per round shown on the table/score panel.
- Bots consider roem preservation, trump, table points, partner-winner status, remaining-card memory, and safer value plays.
...
Asset note:
- A GitHub search for kruip/naakt image assets did not return a usable matching file, so the patch uses a visible text/emoji danger indicator until the exact image asset path is provided or found.

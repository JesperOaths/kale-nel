...
(function(global){
  ...
  async function loadAdminPage(){
    ...
    async function refresh(){
      try {
        const [queue, audit, mints, dry, debts, wallets, liveMarkets] = await Promise.all([
          ...
          adminRpc('admin_get_despimarkt_live_markets_action', { admin_session_token: adminToken(), site_scope_input: scopeValue(), limit_count: 80 })
        ]);
        ...
        if (global.document.getElementById('liveMarketDeleteList')) global.document.getElementById('liveMarketDeleteList').innerHTML = normalizeRows(liveMarkets?.rows).length ? normalizeRows(liveMarkets.rows).map((row)=>`<article class="dm-list-card"><div><strong>${esc(row.title || '')}</strong><div class="muted">${money(row.total_pot || 0)} · ${row.participant_count || 0} spelers · sluit ${shortDate(row.closes_at)}</div><div class="muted">Maker ${playerLink(row.created_by_player_name || '')} · status ${esc(row.status || '')}</div>${tagChips(row.market_tags || [])}${promoBadgeHtml(row)}${promoNoteHtml(row)}</div><div class="admin-actions"><a class="btn alt" href="${route(`./despimarkt_market.html?market=${encodeURIComponent(row.market_id)}`)}">Open</a><button class="btn danger" type="button" data-delete-live-market="${row.market_id}" data-delete-live-title="${esc(row.title || '')}">Verwijder + refund nu</button></div></article>`).join('') : empty('Geen live markten om direct te verwijderen.');
        ...
        global.document.querySelectorAll('[data-delete-live-market]').forEach((btn)=>btn.onclick = async ()=>{
          const title = btn.dataset.deleteLiveTitle || 'deze market';
          const reason = global.prompt(`Reden voor direct verwijderen van "${title}":`, 'Admin quick-delete van live market');
          if (reason === null) return;
          const ok = global.confirm(`Weet je zeker dat je ${title} direct wilt verwijderen? Alle actieve stakes worden terugbetaald en promotionele backing gaat terug naar de promo-wallet.`);
          if (!ok) return;
          setStatus('adminStatus','Live market verwijderen en refunds schrijven…');
          try {
            await adminRpc('admin_cancel_despimarkt_market_action', { admin_session_token: adminToken(), market_id_input: Number(btn.dataset.deleteLiveMarket), reason_input: `[live_delete] ${reason}`, site_scope_input: scopeValue() });
            setStatus('adminStatus','Live market verwijderd en refunds teruggeschreven.','ok');
            await refresh();
          } catch (err) { setStatus('adminStatus', err.message || String(err), 'error'); }
        });
  ...
  async function loadLadderPage(kind){
    ...
    try {
      ...
      const rows = rawRows.map((row)=>({
        ...row,
        player_name: row.player_name || row.display_name || row.public_display_name || '',
        elo: Number(row.elo_rating || row.rating || row.best_rating || 0) || 0,
        games: Number(row.games_played || row.total_matches || row.matches_played || 0) || 0,

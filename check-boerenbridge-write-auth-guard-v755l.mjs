...
assert.match(sql, /revoke\s+insert,\s*update,\s*delete\s+on\s+table\s+public\.boerenbridge_matches\s+from\s+anon,\s*authenticated/i, 'direct public writes to boerenbridge_matches are revoked');
assert.match(sql, /revoke\s+insert,\s*update,\s*delete\s+on\s+table\s+public\.boerenbridge_match_rounds\s+from\s+anon,\s*authenticated/i, 'direct public writes to boerenbridge_match_rounds are revoked');
assert.match(sql, /revoke\s+insert,\s*update,\s*delete\s+on\s+table\s+public\.boerenbridge_player_stats\s+from\s+anon,\s*authenticated/i, 'direct public writes to boerenbridge_player_stats are revoked');

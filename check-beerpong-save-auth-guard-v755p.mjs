...
assert.match(migration, /perform public\.rebuild_beerpong_ratings\(\)/, 'migration uses canonical current-schema rating rebuild');
assert.match(migration, /ratings_rebuilt', v_rebuilt/s, 'migration reports rating rebuild');
assert.match(migration, /'already_saved', v_existing/s, 'migration reports replay state from pre-write lookup');
assert.match(rollback, /ratings_disabled_by_forward_fix/s, 'rollback is forward-fix fallback, not vulnerable restore');

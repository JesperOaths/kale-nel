(function(){
  if (window.GEJAST_PIKKEN_CONTRACT) return;
  ...
  async function createLobby(config){ return rpc(RPC.create, tokenPayload({ config_input: config || {} })); }
  async function joinLobby(code){ return rpc(RPC.join, tokenPayload({ lobby_code_input: cleanCode(code) })); }
  async function getState(gameId, code){ return rpc(RPC.state, tokenPayload({ game_id_input: gameId || null, game_id: gameId || null, lobby_code_input: code ? cleanCode(code) : null })); }
  async function setReady(gameId, ready){ return rpc(RPC.ready, tokenPayload({ game_id_input: gameId, ready_input: !!ready })); }
  async function startGame(gameId){ return rpc(RPC.start, tokenPayload({ game_id_input: gameId })); }
  async function placeBid(gameId, count, face){ return rpc(RPC.bid, tokenPayload({ game_id_input: gameId, bid_count_input: Number(count), bid_face_input: Number(face) })); }
  async function rejectBid(gameId){ return rpc(RPC.reject, tokenPayload({ game_id_input: gameId })); }
  async function castVote(gameId, vote){ return rpc(RPC.vote, tokenPayload({ game_id_input: gameId, vote_input: !!vote })); }
  async function leaveGame(gameId){ return rpc(RPC.leave, tokenPayload({ game_id_input: gameId })); }
  async function destroyGame(gameId){ return rpc(RPC.destroy, tokenPayload({ game_id_input: gameId })); }
  async function openLobbies(){ return rpc(RPC.openLobbies, { site_scope_input: scope() }); }
  async function liveMatches(){ return rpc(RPC.liveMatches, { site_scope_input: scope() }); }
  ...
  window.GEJAST_PIKKEN_CONTRACT = { scope, sessionToken, rpc, cleanCode, createLobby, joinLobby, getState, setReady, startGame, placeBid, rejectBid, castVote, leaveGame, destroyGame, openLobbies, liveMatches, myActive, stats };
})();

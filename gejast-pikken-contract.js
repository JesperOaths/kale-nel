(function(){
  if (window.GEJAST_PIKKEN_CONTRACT && window.GEJAST_PIKKEN_CONTRACT.VERSION === 'v681') return;
  ...
  const RPC = {
    ...
    createFast: 'pikken_create_lobby_fast_v681',
    joinFast: 'pikken_join_lobby_fast_v681',
  ...
  async function rpc(name, payload, timeoutMs){
    ...
    const timer = controller ? setTimeout(()=>{ try{ controller.abort(); }catch(_){} }, Number(timeoutMs || 3600)) : null;
    try {
      ...
      if (err && err.name === 'AbortError') throw new Error('Pikken RPC timeout: server reageert niet snel genoeg. Controleer v681 SQL / Supabase.');
      throw err;
  ...
  async function createLobby(config){ requireSession(); const payload=tokenPayload({ config_input: config || {} }); return rpcFirst([{name:RPC.createFast,payload},{name:RPC.create,payload}], 3600); }
  async function joinLobby(code){ requireSession(); const payload=tokenPayload({ lobby_code_input: cleanCode(code) }); return rpcFirst([{name:RPC.joinFast,payload},{name:RPC.join,payload}], 3600); }
  ...
  async function destroyGame(gameId){ requireSession(); return rpc(RPC.destroy, tokenPayload({ game_id_input: gameId }), 3600); }
  async function openLobbies(){ return rpc(RPC.openLobbies, { site_scope_input: scope() }, 2500); }
  async function liveMatches(){ return rpc(RPC.liveMatches, { site_scope_input: scope() }, 2500); }
  ...
  window.GEJAST_PIKKEN_CONTRACT = { VERSION:'v681', scope, sessionToken, requireSession, rpc, cleanCode, createLobby, joinLobby, getState, setReady, startGame, placeBid, rejectBid, castVote, leaveGame, destroyGame, rpcFirst, openLobbies, liveMatches, myActive, stats };
})();

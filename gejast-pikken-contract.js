(function(){
  if (window.GEJAST_PIKKEN_CONTRACT && window.GEJAST_PIKKEN_CONTRACT.VERSION === 'v687') return;
  ...
  const RPC = {
    ...
    openLobbies: 'get_pikken_open_lobbies_fast_v687',
    liveMatches: 'get_pikken_live_matches_fast_v687',
  ...
  async function rpc(name, payload, timeoutMs){
    ...
    const timer = controller ? setTimeout(()=>{ try{ controller.abort(); }catch(_){} }, Number(timeoutMs || 3500)) : null;
    try {
      ...
      if (err && err.name === 'AbortError') throw new Error('Pikken RPC timeout: server reageert niet snel genoeg. Controleer v687 SQL / Supabase.');
      throw err;
  ...
  async function getState(gameId, code){ return rpc(RPC.state, tokenPayload({ game_id_input: gameId || null, game_id: gameId || null, lobby_code_input: code ? cleanCode(code) : null }), 2800); }
  ...
  async function placeBid(gameId, count, face){ requireSession(); return rpc(RPC.bid, tokenPayload({ game_id_input: gameId, bid_count_input: Number(count), bid_face_input: Number(face) }), 2800); }
  async function rejectBid(gameId){ requireSession(); return rpc(RPC.reject, tokenPayload({ game_id_input: gameId }), 2800); }
  async function castVote(gameId, vote){ requireSession(); return rpc(RPC.vote, tokenPayload({ game_id_input: gameId, vote_input: !!vote }), 2800); }
  async function leaveGame(gameId){ requireSession(); return rpc(RPC.leave, tokenPayload({ game_id_input: gameId }), 2800); }
  async function destroyGame(gameId){ requireSession(); const payload=tokenPayload({ game_id_input: gameId }); return rpcFirst([{name:RPC.destroyFast,payload},{name:RPC.destroy,payload}], 2800); }
  ...
  window.GEJAST_PIKKEN_CONTRACT = { VERSION:'v687', scope, sessionToken, requireSession, rpc, cleanCode, createLobby, joinLobby, getState, setReady, startGame, placeBid, rejectBid, castVote, leaveGame, destroyGame, rpcFirst, openLobbies, liveMatches, myActive, stats };
})();

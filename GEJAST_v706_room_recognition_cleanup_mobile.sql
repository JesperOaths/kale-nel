-- GEJAST v706: room recognition fallback, stale-room cleanup, Pikken reveal/mobile polish.
...
begin
  if to_regclass('public.pikken_games') is null then
    return jsonb_build_object('ok', false, 'reason', 'pikken_games_missing', 'closed', 0);
  end if;
  ...
  execute $sql$
    ...
    where coalesce(updated_at, created_at, now()) < $1
      and coalesce(status, state->>'phase', 'lobby') not in ('finished','deleted','closed')
      and public._gejast_scope_norm_v702(coalesce(site_scope,'friends')) = public._gejast_scope_norm_v702($2)
  $sql$ into v_ids using v_cutoff, site_scope_input;
  ...
  execute $sql$
    update public.pikken_games
       set status = 'deleted',
           state = coalesce(state, '{}'::jsonb)
             || jsonb_build_object('phase','deleted','deleted_reason','inactive_15m','closed_at',now()),
           updated_at = now()
     where id = any($1)
  $sql$ using v_ids;

  if exists(select 1 from information_schema.columns where table_schema='public' and table_name='pikken_games' and column_name='finished_at') then
    execute 'update public.pikken_games set finished_at = coalesce(finished_at, now()) where id = any($1)' using v_ids;
  end if;
  if exists(select 1 from information_schema.columns where table_schema='public' and table_name='pikken_games' and column_name='deleted_at') then
    execute 'update public.pikken_games set deleted_at = coalesce(deleted_at, now()) where id = any($1)' using v_ids;
  end if;
  if to_regclass('public.pikken_round_votes') is not null then
    execute 'delete from public.pikken_round_votes where game_id = any($1)' using v_ids;
  end if;
...
create or replace function public.cleanup_stale_paardenrace_rooms_v706(site_scope_input text default 'friends')
returns jsonb
language plpgsql
security definer
set search_path to 'public'
...
begin
  ...
  if exists(select 1 from information_schema.columns where table_schema='public' and table_name='paardenrace_rooms' and column_name='stage') and exists(select 1 from information_schema.columns where table_schema='public' and table_name='paardenrace_rooms' and column_name='status') then
    v_state_expr := 'coalesce(stage, status, ''lobby'')';
    ...
    v_state_expr := 'coalesce(stage, ''lobby'')';
    ...
    v_state_expr := 'coalesce(status, ''lobby'')';
  end if;
  ...
  execute format($sql$
    ...
    where coalesce(updated_at, created_at, now()) < $1
      and %s not in ('closed','deleted','archived')
      and public._gejast_scope_norm_v702(%s) = public._gejast_scope_norm_v702($2)
  $sql$, v_state_expr, v_scope_expr) into v_ids using v_cutoff, site_scope_input;
  ...
  if exists(select 1 from information_schema.columns where table_schema='public' and table_name='paardenrace_rooms' and column_name='stage') then
    ...
    execute 'update public.paardenrace_rooms set updated_at = now() where id = any($1)' using v_ids;
  end if;
  ...
  if exists(select 1 from information_schema.columns where table_schema='public' and table_name='paardenrace_rooms' and column_name='deleted_at') then
    execute 'update public.paardenrace_rooms set deleted_at = coalesce(deleted_at, now()) where id = any($1)' using v_ids;
  end if;
  if to_regclass('public.paardenrace_room_players') is not null then
    execute 'delete from public.paardenrace_room_players where room_id = any($1)' using v_ids;
  end if;

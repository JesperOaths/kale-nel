(() => {
  ...
  function transparentMedia(product, liveProduct){
    ...
    const seen = new Set();
    const transparent = candidates.filter(item => {
      const url = imageUrl(item);
      if(!url || !isTransparentAsset(url) || seen.has(url)) return false;
      seen.add(url);
      return true;
    }).map((item, index) => ({
      label: String(item?.label || (index === 0 ? 'Front artwork' : `Artwork ${index + 1}`)),
      image: imageUrl(item)
    }));

    if(transparent.length) return transparent;
    const first = candidates.find(item => imageUrl(item));
    return first ? [{ label: String(first.label || 'Front'), image: imageUrl(first) }] : [];
  }
  ...
  style.dataset.shopTransparentListingsV818 = 'true';
  style.textContent = `
    .mockup-rail {

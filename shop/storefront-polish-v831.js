(() => {
  ...
  const ARTWORK_ONLY_RE = /(?:artwork|front\s*print|standalone\s*print|print\s*only)/i;
  const wholeEuro = value => Math.ceil(Math.max(0, Number(value || 0)) - 1e-9);

  function isArtwork(item){
    const label = String(item?.label || '');
    const image = String(item?.image || '');
    return ARTWORK_ONLY_RE.test(label) || /(?:^|[-_/])front-(?:artwork|print)(?:[-_.?/]|$)/i.test(image);
  }
  ...
  function sanitizeProduct(product){
    ...
    if(views.length){
      ...
      const primaryGarment = views.find(item => !isArtwork(item));
      product.image = primaryGarment?.image || views[0].image || product.image || '';
    }
    return product;
  }
  ...
  const observer = new MutationObserver(records => {
    if(records.some(record => record.type === 'characterData' || record.addedNodes?.length)){
      requestAnimationFrame(() => polishCheckout(document));
    }
  });
  observer.observe(document.documentElement, { childList: true, subtree: true, characterData: true });
  window.addEventListener('pageshow', normalizeCurrentState, { once: true });

  window.BRUIS_STOREFRONT_V831 = Object.freeze({
    wholeEuroPricing: true,
    artworkDetailsRestored: true,
    garmentPrimaryImage: true,

(() => {
  ...
  const style=document.createElement('style');style.dataset.shopLightboxV832='true';style.textContent=`
    ...
    .shop-lightbox-v832-media{--lb-pad-x:clamp(28px,4vw,60px);--lb-pad-y:clamp(28px,4vw,60px);position:relative;grid-column:2;grid-row:1;width:100%;height:100%;min-height:0;display:block;overflow:hidden;padding:0;border-radius:16px;background:#ded6ca;touch-action:none;user-select:none}
    .shop-lightbox-v832-media img{position:absolute!important;inset:var(--lb-pad-y) var(--lb-pad-x)!important;display:block;width:calc(100% - var(--lb-pad-x) - var(--lb-pad-x))!important;height:calc(100% - var(--lb-pad-y) - var(--lb-pad-y))!important;max-width:none!important;max-height:none!important;object-fit:contain!important;object-position:center!important;background:transparent!important;transform:translate3d(0,0,0) scale(1);transform-origin:center;user-select:none;-webkit-user-drag:none;transition:transform .1s ease-out}

...
begin
  for rec in
    ...
    join pg_namespace n on n.oid = p.pronamespace
    where n.nspname = 'public'
      and p.proname = 'cleanup_stale_paardenrace_rooms_v706'
  loop
    execute format('drop function if exists %I.%I(%s) cascade', rec.nspname, rec.proname, rec.args);
  end loop;
end
...
set search_path to 'public'
as $fn$
  select public.cleanup_stale_paardenrace_rooms_v718(site_scope_input)
$fn$;
...
grant execute on function public.cleanup_stale_paardenrace_rooms_v706(text) to anon, authenticated;

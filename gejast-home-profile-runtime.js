(function(){
  ...
  async function loadPlayer(){ const p=new URLSearchParams(location.search).get('player')||''; if(!p) return null; const data=await rpc(RPC.player,{player_name_input:p,player_input:p,site_scope_input:scope()}); renderPlayer(data); return data; }

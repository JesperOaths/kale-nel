...
const adminAnalyticsEdge = read('supabase/functions/shop-admin-analytics-v841/index.ts');
...
assert.match(index, /version-watermark[^>]*>v841</);
...
assert.match(catalogEdge, /source:\s*"bruis-direct-v838"/);
...
assert.match(checkoutEdge, /PRINTIFY_FULFILLMENT_MAPPINGS/);
...
assert.match(adminAnalyticsPage, /shop-admin-analytics-v841/);
...
assert.match(deployWorkflow, /supabase\/functions\/shop-admin-analytics-v841\/\*\*/);
...
assert.match(deployWorkflow, /deploy_function shop-admin-analytics-v841/);

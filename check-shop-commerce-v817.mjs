...
assert.match(styles, /\.product-grid\s*\{[\s\S]*display:\s*flex[\s\S]*justify-content:\s*center/, 'product rows must center incomplete last rows');
assert.match(styles, /flex:\s*0 1 calc\(\(100% - 44px\) \/ 3\)/, 'desktop cards must retain three-column sizing while last rows center');
assert.match(styles, /flex-basis:\s*calc\(\(100% - 22px\) \/ 2\)/, 'tablet rows must center two cards');
assert.match(styles, /flex-basis:\s*100%/, 'single-column mobile rows must remain full width');

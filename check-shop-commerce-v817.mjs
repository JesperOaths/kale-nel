...
assert.match(index, /version-watermark[^>]*>v869</);
...
assert.match(styles, /\.product-grid\s*\{[\s\S]*display:\s*grid[\s\S]*grid-template-columns:\s*repeat\(3, minmax\(0, 1fr\)\)/, 'desktop product sections must use an exact three-slot grid');
assert.match(styles, /last-child:nth-child\(3n \+ 1\)[\s\S]*grid-column:\s*2/, 'a one-item desktop last row must occupy the middle slot');
assert.match(styles, /last-child:nth-child\(3n \+ 2\)[\s\S]*grid-column:\s*3/, 'a two-item desktop last row must leave the middle slot empty by placing its last item on the right');

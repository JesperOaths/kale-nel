...
// The perfume page is deliberately an admin/TOTP login shell rather than a
// player-session page. Collection rows are not embedded in the published HTML;
// they are returned only after admin_login and an authenticated edge-function
// call. Treat that independent admin data boundary like the security surface,
// while asserting the critical properties here so this exception cannot become
// an accidental public-data bypass.
...
assert(!perfumeBody.includes('/gejast-auth-gate.js?'),'perfume admin login shell must not depend on player-session gate');
for(const required of ['/rest/v1/rpc/admin_login','/functions/v1/perfume-dashboard','admin_session_token','6-digit authenticator code']) assert(perfumeBody.includes(required),`perfume admin/TOTP boundary missing required contract: ${required}`);
assert(!/const\s+items\s*=\s*\[[\s\S]*\{\s*rank\s*:/i.test(perfumeBody),'private perfume collection rows must not be embedded in published HTML');
...
console.log('v798 forced-login publication boundary ok; gated app pages=',protectedCount,'runtime-light aliases=',redirectOnly.size,'separate_security_perimeter=1','separate_perfume_admin_totp_data_perimeter=1');

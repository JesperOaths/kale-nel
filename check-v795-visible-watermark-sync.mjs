...
function isArchivedFile(rel){
  ...
  if (/^gejast-v\d+-repair\.js$/i.test(base) && !base.toLowerCase().includes(version.toLowerCase())) return true;
...
for (const file of walk(process.cwd())) {
  ...
  if (isArchivedFile(rel) || ignoredFiles.has(path.basename(file)) || !activeExt.has(path.extname(file).toLowerCase())) continue;
  const body = fs.readFileSync(file,'utf8');
  for (const match of body.matchAll(/v(\d+)\s*·\s*Made by Bruis/gi)) {

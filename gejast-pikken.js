(function(){
  ...
  const PIKKEN_LEAVE_SUPPRESS_KEY = 'gejast_pikken_leave_suppress_v552';
  const PIKKEN_VIEWER_HINT_KEY = 'gejast_pikken_viewer_hint_v552';
  const PIKKEN_STATE_SNAPSHOT_KEY = 'gejast_pikken_state_snapshot_v552';
  const PIKKEN_LEAVE_SUPPRESS_LEGACY_KEYS = ['gejast_pikken_leave_suppress_v540'];
  const PIKKEN_VIEWER_HINT_LEGACY_KEYS = ['gejast_pikken_viewer_hint_v541','gejast_pikken_viewer_hint_v542'];
  const PIKKEN_STATE_SNAPSHOT_LEGACY_KEYS = ['gejast_pikken_state_snapshot_v541','gejast_pikken_state_snapshot_v542'];
  ...
  function setParticipantToken(gameId, active, lobbyCode, seat){ try{ if(active && gameId){ localStorage.setItem(PIKKEN_PARTICIPANT_KEY, JSON.stringify({game_id:String(gameId), lobby_code:String(lobbyCode||'').trim().toUpperCase(), seat:(Number(seat||0) > 0 ? Number(seat) : null), at:Date.now()})); } else { localStorage.removeItem(PIKKEN_PARTICIPANT_KEY); } }catch(_){ } }
  ...
  function getLeaveSuppression(){ return readJsonFromKeys([PIKKEN_LEAVE_SUPPRESS_KEY, ...PIKKEN_LEAVE_SUPPRESS_LEGACY_KEYS]); }
  ...
  function getViewerHint(){ return readJsonFromKeys([PIKKEN_VIEWER_HINT_KEY, ...PIKKEN_VIEWER_HINT_LEGACY_KEYS]); }
  ...
  function getStateSnapshot(){ return readJsonFromKeys([PIKKEN_STATE_SNAPSHOT_KEY, ...PIKKEN_STATE_SNAPSHOT_LEGACY_KEYS]); }
  ...
  function liveHref(gameId, lobbyCode, seatHint){
    const id = String(gameId || '').trim();
    const code = String(lobbyCode || '').trim().toUpperCase();
    const seat = Number(seatHint || 0);
    const params = {};
    if (id) params.client_match_id = id;
    if (code) params.match_ref = code;
    if (seat > 0) params.seat = String(seat);
  ...
  function goLive(gameId, lobbyCode, seatHint){
    const id = String(gameId || UI.gameId || '').trim();
    const code = String(lobbyCode || currentLobbyCode() || '').trim().toUpperCase();
    const seat = Number(seatHint || UI.state?.viewer?.seat || getViewerHint()?.seat || 0);
    if(!id && !code) return false;
    window.location.href = liveHref(id, code, seat > 0 ? seat : null);
    return true;
  }
  ...
  function applyCreateFallback(out){
    ...
    if(gameId){
      UI.gameId = gameId;
      setParticipantToken(gameId, true, lobbyCode, null);
      ...
      btn.addEventListener('click', async()=>{
        ...
        if(stage && stage !== 'lobby'){
          ...
          if(resolvedGameId){
            ...
            setParticipantToken(resolvedGameId, true, code, null);
          ...
          goLive(resolvedGameId || UI.gameId || '', code, getViewerHint()?.seat || null);
          return;
        }
      ...
      if (matchedLive && !/pikken_live\.html/i.test(window.location.pathname) && hasParticipantClaimFor(matchedLive)) {
        ...
        if (gameId) {
          ...
          if (joinedViewer && joinedPhase && joinedPhase !== 'lobby') {
            ...
            setParticipantToken(gameId || UI.gameId || '', true, lobbyCode, joinedState?.viewer?.seat || null);
            clearLeaveSuppressionFor(gameId || UI.gameId || '', lobbyCode);
            history.replaceState(null,'',lobbyHref(gameId || UI.gameId || ''));
            goLive(gameId || UI.gameId || '', lobbyCode || currentLobbyCode(), joinedState?.viewer?.seat || getViewerHint()?.seat || null);
            return list;
          }
        }
      }
  ...
  function render(state){
    ...
    setParticipantToken(game?.id || UI.gameId, joinedViewer && phase && phase !== 'finished', game?.lobby_code || currentLobbyCode(), viewer?.seat || getViewerHint()?.seat || null);
    ...
    const liveLink = qs('#pkLiveLink'); if(liveLink){ liveLink.href = liveHref(game?.id || UI.gameId, game?.lobby_code || currentLobbyCode(), viewer?.seat || getViewerHint()?.seat || null); liveLink.style.display = (game?.id || UI.gameId || game?.lobby_code || currentLobbyCode()) ? '' : 'none'; }
  ...
  async function startGame(){
    ...
    if(joinedViewer && phase && phase !== 'lobby'){
      ...
      setStatus('', false);
      goLive(String(stateToUse?.game?.id || UI.gameId || '').trim(), stateToUse?.game?.lobby_code || currentLobbyCode(), stateToUse?.viewer?.seat || getViewerHint()?.seat || null);
      return;
    }
    ...
    if(confirmedViewer && confirmedPhase && confirmedPhase !== 'lobby'){
      ...
      setStatus('', false);
      goLive(String(confirmedState?.game?.id || UI.gameId || '').trim(), confirmedState?.game?.lobby_code || currentLobbyCode(), confirmedState?.viewer?.seat || getViewerHint()?.seat || null);
      return;
    }

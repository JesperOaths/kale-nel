(function(){
  ...
  const PARTICIPANT_KEY = 'gejast_pikken_participant_v685';
  ...
  function normalizeError(e){ const msg=String(e && e.message ? e.message : e || 'Onbekende fout'); if(/Niet ingelogd|not logged|session/i.test(msg)) return `Niet ingelogd. Log eerst in via ${loginUrl()}`; if(/timeout/i.test(msg)) return 'Server reageert niet op de Pikken-RPC. Controleer of de laatste v685 SQL heeft gedraaid.'; if(/could not find|schema cache|function/i.test(msg)) return 'Pikken backend-RPC ontbreekt of Supabase schema cache is nog oud. Draai de v685 SQL en refresh.'; return msg; }

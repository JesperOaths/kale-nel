(function(){
  const CONFIG = {
    ...
    ACTIVE_USERNAME_STATUSES: ['active','activated','approved']
  };
  ...
  function watermarkStyles(node){
    ...
    Object.assign(node.style, {
      ...
      border: '1px solid rgba(212,175,55,0.35)', color: '#f3e3a6', font: compact ? '700 12px/1.2 Inter,system-ui,sans-serif' : '700 13px/1.2 Inter,system-ui,sans-serif',
  ...
  function isActiveUsernameStatus(value){ return CONFIG.ACTIVE_USERNAME_STATUSES.includes(String(value || '').trim().toLowerCase()); }

  async function fetchScopedActivePlayerNames(scope){
    const resolvedScope = normalizeScope(scope || inferRuntimeScope());
    const authHeaders = { apikey: CONFIG.SUPABASE_PUBLISHABLE_KEY || '', Authorization:`Bearer ${CONFIG.SUPABASE_PUBLISHABLE_KEY || ''}`, Accept:'application/json' };
    const url = `${CONFIG.SUPABASE_URL}/rest/v1/allowed_usernames?select=display_name,slug,status,site_scope&order=display_name.asc&limit=600`;
    const rows = await callJson(url, { method:'GET', mode:'cors', cache:'no-store', headers: authHeaders }, CONFIG.LOGIN_NAME_TIMEOUT_MS).catch(()=>[]);
    let filtered = (Array.isArray(rows) ? rows : []).filter((row)=>{
      const statusOk = isActiveUsernameStatus(row?.status);
      const scopeRaw = String(row?.site_scope || '').trim().toLowerCase();
      const scopeOk = !scopeRaw || scopeRaw === resolvedScope;
      return statusOk && scopeOk;
    });
    if (window.GEJAST_SCOPE_UTILS && typeof window.GEJAST_SCOPE_UTILS.filterNames === 'function') {
      return window.GEJAST_SCOPE_UTILS.filterNames(uniquePersonNames(filtered.map((row)=>row?.display_name || row?.slug || '')), resolvedScope);
    }
    return uniquePersonNames(filtered.map((row)=>row?.display_name || row?.slug || ''));
  }
  ...
  function lastPlayerActivity(){ const raw = localStorage.getItem(CONFIG.PLAYER_LAST_ACTIVITY_KEY) || sessionStorage.getItem(CONFIG.PLAYER_LAST_ACTIVITY_KEY) || ''; const n = Number(raw || 0); return Number.isFinite(n) ? n : 0; }
  function isPlayerSessionExpired(){ const token = getPlayerSessionToken(); if (!token) return true; const last = lastPlayerActivity(); if (!last) return false; return (Date.now() - last) > CONFIG.PLAYER_SESSION_IDLE_MS; }
  function isPlayerSessionLocallyStale(){ const token = getPlayerSessionToken(); if (!token) return true; const last = lastPlayerActivity(); if (!last) return false; return (Date.now() - last) > CONFIG.PLAYER_SESSION_SOFT_STALE_MS; }
  ...
  function setPlayerSessionToken(token, storage){ const value = String(token || '').trim(); if (!value) return ''; const primaryKey = CONFIG.PLAYER_SESSION_KEYS[0] || 'jas_session_token_v11'; const target = storage === 'session' ? sessionStorage : localStorage; target.setItem(primaryKey, value); const other = target === localStorage ? sessionStorage : localStorage; other.removeItem(primaryKey); touchPlayerActivity(); return value; }
  function buildRequestUrl(returnTo, scope){ const normalizedScope = normalizeScope(scope || inferRuntimeScope()); const url = new URL('./request.html', window.location.href); const safeTarget = sanitizeReturnTarget(returnTo || '', normalizedScope === 'family' ? 'index.html?scope=family' : 'index.html'); if (safeTarget) url.searchParams.set('return_to', safeTarget); if (normalizedScope === 'family') url.searchParams.set('scope', 'family'); return `${url.pathname}${url.search}${url.hash}`; }
  function buildAdminUrl(reason='', returnTo=''){ const url = new URL('./admin.html', window.location.href); if (reason) url.searchParams.set('reason', String(reason)); const target = sanitizeReturnTarget(returnTo); if (target) url.searchParams.set('return_to', target); return url.toString(); }
  function ensurePlayerSessionOrRedirect(returnTo){ const token = getPlayerSessionToken(); if (!token){ const target = sanitizeReturnTarget(returnTo || (location.pathname.split('/').pop() || 'index.html'), 'index.html'); window.location.href = buildHomeUrl(target); return false; } touchPlayerActivity(); return true; }
  function installActivityKeepalive(options={}){ const suppressLogout = !!options.suppressLogout; const logoutSelectors = options.logoutSelectors || ['#playerSessionCornerLogout','.player-session-corner-btn.logout','[data-player-logout]']; const touch = ()=> touchPlayerActivity(); ['pointerdown','keydown','scroll','touchstart','mousemove'].forEach((eventName)=> window.addEventListener(eventName, touch, { passive:true })); document.addEventListener('visibilitychange', ()=>{ if (!document.hidden) touch(); }); window.addEventListener('focus', touch); touch(); if (suppressLogout){ logoutSelectors.forEach((selector)=>{ document.querySelectorAll(selector).forEach((el)=>{ el.style.display = 'none'; el.setAttribute('aria-hidden','true'); el.disabled = true; }); }); } }
  function requireMatchEntrySession(returnTo){ const ok = ensurePlayerSessionOrRedirect(returnTo); if (!ok) return false; if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', ()=>installActivityKeepalive({ suppressLogout:true }), { once:true }); else installActivityKeepalive({ suppressLogout:true }); return true; }
  ...
  window.GEJAST_CONFIG = Object.assign({}, window.GEJAST_CONFIG || {}, CONFIG, {
    ...
    fetchScopedActivePlayerNames, getPlayerSessionToken, clearPlayerSessionTokens, touchPlayerActivity, lastPlayerActivity,
    isPlayerSessionExpired, isPlayerSessionLocallyStale, ensurePlayerSessionOrRedirect, installActivityKeepalive,
    requireMatchEntrySession, buildHomeUrl, buildLoginUrl, buildAdminUrl, setPlayerSessionToken, buildRequestUrl,
    normalizeScope, sanitizeReturnTarget, currentReturnTarget, isActiveUsernameStatus
  });

-- v755r Klaverjas post-apply verification. READ ONLY.
...
insert into _v755r_post
select 'controlled_push_jobs',case when count(*)=0 then 'PASS' else 'FAIL' end,'OC_V765 push rows='||count(*)::text
  from public.web_push_jobs where to_jsonb(web_push_jobs)::text ilike '%OC_V765%';

...
insert into public.implementation_matrix_features (
  ...
  ('critical.make_webhook_safety', 0, 'email-make', 'Make webhook safety and browser exposure removal', 'critical_planned', 'admin_mail_audit.html', 'gejast-config.js', 'claim_email_jobs_http', 'email_queue_validation_patch', 'browser_webhook_still_exposed', 'unknown', 'repo_gap_identified', 'critical', 'Known critical architecture item: backend must validate email jobs before Make and browser must not own Make webhook invocation.', 'Backend-validate email jobs before Make; do not let browser own Make webhook invocation.', true),
  ...
  ('phase4.system_health_version_gate_boot_perf', 4, 'system-health', 'System health, versioning, gate, boot and performance hardening', 'planned_partial_foundation', 'admin.html', 'gejast-config.js', 'not_yet_canonical', 'not_yet_canonical', 'partial_foundations_present', 'unknown', 'inferred_from_repo', 'critical', 'Current repo has config/gate foundations but no durable audit substrate yet.', 'Implement after Phase 3 SQL is live.', true),

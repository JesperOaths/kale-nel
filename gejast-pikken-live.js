(function(){
  ...
  function seatClass(index, total){
    ...
    const map8 = ['top','extra2','right','extra3','bottom','extra4','left','extra1'];
    const table = total <= 4 ? map4 : total <= 6 ? map6 : map8;
    return table[index] || `extra${(index % 4) + 1}`;
  }
  ...
  async function touchPresence(){
    ...
    try {
      await rpcVariants('pikken_touch_presence_scoped', [
        { session_token: token, game_id_input: gameId, page_kind_input: 'live', site_scope_input: getScope() },
        { session_token: token, game_id_input: gameId, page_kind_input: 'live' }
      ]);
    } catch (_) {}
  }
  async function releasePresence(){
    ...
    try {
      await rpcVariants('pikken_release_presence_scoped', [
        { session_token: token, game_id_input: gameId, site_scope_input: getScope() },
        { session_token: token, game_id_input: gameId }
      ]);
    } catch (_) {}
  }
  async function loadActorState(){
    const token = sessionToken();
    if (!token || !gameId) return null;
    try {
      return await rpcVariants('pikken_get_state_scoped', [
        { session_token: token, game_id_input: gameId },
        { session_token: token, game_id_input: gameId, site_scope_input: getScope() }
      ]);
    } catch (_) { return null; }
  ...
  function renderMyDice(model){
    ...
    wrap.innerHTML = groupDice(dice).map((group)=>`
      <div class="dice-group">
        ...
        <div class="dice-row">${group.dice.map((face)=>`<img class="die" src="./assets/pikken/dice-${Number(face)}.svg" alt="die ${Number(face)}">`).join('')}</div>
      </div>
    `).join('');
  }
      ...
      <div class="reveal-grid">
        ${(Array.isArray(lastReveal.hands) ? lastReveal.hands : []).map((h)=>`
          <div class="reveal-card">
            <div><strong>${esc(h.name || 'Speler')}</strong> <span class="subline">#${Number(h.seat || 0)}</span></div>
  ...
  function renderSeats(model){
    ...
    Array.from(ring.querySelectorAll('.seat')).forEach((node)=>node.remove());
    const players = model.players || [];
    players.forEach((p, idx)=>{
      const seat = Number(p.seat || p.seat_index || 0);
      const meSeat = Number(model.viewer?.seat || model.viewer?.seat_index || 0);
      const alive = !!p.alive || String(model.phase || '').toLowerCase() === 'lobby';
      const presence = model.presenceMap[String(p.name || p.player_name || '').trim().toLowerCase()] || null;
      const disconnected = !!presence && presence.connected === false;
      const seatHistory = UI.bidHistory.filter((row)=>Number(row.round_no || 0) === Number(model.roundNo || 0) && Number(row.seat || 0) === seat).slice(-3);
      const vote = (model.votes || []).find((row)=>Number(row.seat || row.seat_index || 0) === seat);
      const voteIcon = model.phase === 'voting' ? (vote?.status === 'approved' ? '👍' : vote?.status === 'rejected' ? '👎' : '…') : '';
      const canKick = model.mode === 'actor' && !!model.viewer?.is_host && meSeat !== seat;
      const div = document.createElement('div');
      div.className = `seat ${seatClass(idx, players.length)} ${seat === meSeat ? 'me' : ''} ${(seat === model.turnSeat || seat === model.voteTurnSeat) ? 'turn' : ''} ${alive ? '' : 'dead'} ${disconnected ? 'disconnected' : ''}`.trim();
      div.innerHTML = `
        <div class="seat-top">
          <div>
            <div class="seat-name">${esc(p.name || p.player_name || 'Speler')}</div>
            <div class="seat-meta">stoel ${seat || '—'} · ${Number(p.dice_count || 0)} dobbel(s)${disconnected ? ' · offline' : ''}</div>
          </div>
          ${canKick ? `<button class="seat-kick" type="button" data-kick-seat="${seat}">kick</button>` : ''}
        </div>
        <div class="seat-badges">
          ${seat === model.turnSeat && model.phase === 'bidding' ? '<span class="pill ok">aan zet</span>' : ''}
          ${seat === model.voteTurnSeat && model.phase === 'voting' ? '<span class="pill wait">stemt nu</span>' : ''}
          ${!alive ? '<span class="pill bad">uit</span>' : ''}
          ${voteIcon ? `<span class="pill ${vote?.status === 'approved' ? 'ok' : vote?.status === 'rejected' ? 'bad' : 'wait'}">${voteIcon}</span>` : ''}
        </div>
        <div class="seat-history">${seatHistory.length ? seatHistory.map((row)=>`<span>${esc(row.label)}</span>`).join('') : '<span>—</span>'}</div>
      `;
  ...
  function renderModel(model){
    ...
    phasePill.textContent = model.phase || 'live';
    phasePill.className = `pill ${model.phase === 'finished' ? 'bad' : model.phase === 'bidding' ? 'ok' : 'wait'}`;
    qs('#currentBid').textContent = bidText(model.bid);
    qs('#bidBy').textContent = model.bid ? `door ${model.bid.bidder_name || model.players.find((p)=>Number(p.seat || p.seat_index || 0) === Number(model.bid.bidder_seat || model.bid.seat || 0))?.name || '—'}` : 'Nog geen bod';
    qs('#roundNo').textContent = String(Number(model.roundNo || 0));
    const activeSeat = model.phase === 'voting' ? model.voteTurnSeat : model.turnSeat;
    ...
    const myVoteTurn = model.mode === 'actor' && model.phase === 'voting' && mySeat === model.voteTurnSeat && meAlive;
    qs('#mobileContext').textContent = myTurn ? 'Kies een legale volgende bieding of keur af.' : (myVoteTurn ? 'Stem zichtbaar met een duim.' : (model.mode === 'viewer' ? 'Spectator-modus.' : 'Volg de match live.'));
    qs('#bidControls').classList.toggle('hidden', !myTurn);
    qs('#voteControls').classList.toggle('hidden', !myVoteTurn);
    qs('#rejectBtn').disabled = !myTurn || !model.bid;
  ...
  async function act(name, payload){
    await rpcVariants(name, [
      ...
      Object.assign({ session_token: sessionToken(), game_id_input: gameId, site_scope_input: getScope() }, payload || {})
    ]);
    await refresh();
  }
  ...
  UI.pollId = setInterval(()=>{ if (!document.hidden) refresh().catch(()=>{}); }, 1500);
  UI.presenceId = setInterval(()=>{ if (!document.hidden) touchPresence().catch(()=>{}); }, 8000);

(function(){
  ...
  const BID_HISTORY_KEY = 'jas_pikken_bid_history_v506';
  ...
  function renderModel(model){
    ...
    const bc=qs('#bidControls'); if(bc) bc.classList.toggle('hidden', !myTurn); const vc=qs('#voteControls'); if(vc) vc.classList.toggle('hidden', !myVoteTurn); const rejectBtn=qs('#rejectBtn'); if(rejectBtn) rejectBtn.disabled=!myTurn || !model.bid; const leaveBtn=qs('#leaveBtn'); if(leaveBtn) leaveBtn.classList.toggle('hidden', !(model.mode==='actor' && (model.viewer?.player_id || model.viewer?.display_name || model.viewer?.player_name))); const destroyBtn=qs('#destroyBtn'); if(destroyBtn) destroyBtn.classList.toggle('hidden', !(model.mode==='actor' && !!model.viewer?.is_host));
  }
  ...
  UI.pollId = setInterval(()=>{ if (!document.hidden) refresh().catch(()=>{}); }, 2600);

(function(){
  ...
  const VERSION='v659';
  const RPC={
    audit:'admin_get_game_group_b_audit_v659',
    generic:'get_game_group_b_bundle_v659',
    pikken:'get_pikken_phase_bundle_v659',
    paardenrace:'get_paardenrace_phase_bundle_v659'
  };
  ...
  function renderPanel(data,target){
    ...
    body.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:8px;margin-bottom:10px"><div><b>${escapeHtml(totals.games||0)}</b><br><span>games</span></div><div><b>${escapeHtml(totals.players||0)}</b><br><span>spelers/entries</span></div><div><b>${escapeHtml(totals.active||0)}</b><br><span>actief/live</span></div><div><b>${escapeHtml(data.source||'v659')}</b><br><span>bron</span></div></div>`+

-- GEJAST v723 Paardenrace no ambiguous safe-call SQL
-- Fixes:
...
create or replace function public._paardenrace_next_despinoza_room_code_v723()
returns text
language plpgsql
security definer
...
begin
  ...
  loop
    ...
    v_code := public._paardenrace_next_despinoza_room_code_v723();
    begin
...
create or replace function public._pikken_state_public_v723(
...
set search_path to 'public'
as $fn$
  select public._pikken_state_public_v723(game_id_input, null, lobby_code_input, session_token, session_token_input, site_scope_input)
$fn$;
...
set search_path to 'public'
as $fn$
  select public._pikken_state_public_v723(game_id_input, client_match_id, lobby_code_input, session_token, session_token_input, site_scope_input)
$fn$;
...
set search_path to 'public'
as $fn$
  select public._pikken_state_public_v723(game_id_input, null, lobby_code_input, null, null, site_scope_input)
$fn$;

grant execute on function public._paardenrace_next_despinoza_room_code_v723() to anon, authenticated;
...
grant execute on function public._pikken_state_public_v723(uuid,uuid,text,text,text,text) to anon, authenticated;

...
create or replace function public._paardenrace_next_despinoza_room_code_v720()
returns text
language plpgsql
security definer
...
begin
  ...
  loop
    ...
    v_code := public._paardenrace_next_despinoza_room_code_v720();
    begin
...
create or replace function public._pikken_state_public_v720(
...
begin
  ...
  loop
    v_players := v_players || jsonb_build_array(jsonb_build_object(
      ...
      'is_ready', coalesce(r.is_ready,false),
      'ready', coalesce(r.is_ready,false),
...
set search_path to 'public'
as $fn$
  select public._pikken_state_public_v720(game_id_input, game_id, null, lobby_code_input, session_token, session_token_input, site_scope_input)
$fn$;
...
set search_path to 'public'
as $fn$
  select public._pikken_state_public_v720(game_id_input, null, client_match_id, lobby_code_input, session_token, session_token_input, site_scope_input)
$fn$;
...
set search_path to 'public'
as $fn$
  select public._pikken_state_public_v720(game_id_input, null, null, lobby_code_input, null, null, site_scope_input)
$fn$;

grant execute on function public._paardenrace_next_despinoza_room_code_v720() to anon, authenticated;
...
grant execute on function public._pikken_state_public_v720(uuid,uuid,uuid,text,text,text,text) to anon, authenticated;

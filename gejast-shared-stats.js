(function(){
  ...
  const CACHE_KEY=CFG.cache_key||'gejast_shared_stats_cache_v660';
  ...
  async function summary(opts={}){ const payload={site_scope_input:scope(opts.scope),game_key_input:opts.gameKey||opts.game_key||null}; return writeCache(cacheKey('summary',opts),await rpc(rpcNames.summary||'get_shared_stats_summary_v660',payload)); }
  async function leaderboard(opts={}){ const payload={site_scope_input:scope(opts.scope),game_key_input:opts.gameKey||opts.game_key||null,limit_input:limit(opts.limit)}; return writeCache(cacheKey('leaderboard',opts),await rpc(rpcNames.leaderboard||'get_shared_stats_leaderboard_v660',payload)); }
  async function crossGame(opts={}){ const payload={site_scope_input:scope(opts.scope),limit_input:limit(opts.limit)}; return writeCache(cacheKey('crossGame',opts),await rpc(rpcNames.crossGame||'get_cross_game_player_summary_v660',payload)); }
  async function klaverjas(opts={}){ const payload={site_scope_input:scope(opts.scope),limit_input:limit(opts.limit)}; return writeCache(cacheKey('klaverjas',opts),await rpc(rpcNames.klaverjas||'get_klaverjas_shared_stats_v660',payload)); }
  async function adminAudit(opts={}){ return await rpc(rpcNames.adminAudit||'admin_get_shared_stats_audit_v660',{site_scope_input:scope(opts.scope)}); }
  async function refresh(opts={}){ return await rpc(rpcNames.refresh||'refresh_shared_stats_cache_v660',{site_scope_input:scope(opts.scope)}); }

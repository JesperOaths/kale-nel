(function(){
  ...
  function setSession(v, u, d){
    if (v) { sessionStorage.setItem(ADMIN_SESSION_KEY, v); localStorage.setItem(ADMIN_SESSION_KEY, v); }
    if (u) { sessionStorage.setItem(ADMIN_USER_KEY, u); localStorage.setItem(ADMIN_USER_KEY, u); }
    if (d) localStorage.setItem(ADMIN_DEVICE_KEY, d);
  ...
  function fingerprint(){
    const p = [navigator.userAgent || '', navigator.language || '', Intl.DateTimeFormat().resolvedOptions().timeZone || '', String(screen?.width || 0), String(screen?.height || 0), navigator.platform || ''];
  ...
  function addLogout(){
    if (document.getElementById('globalAdminLogoutBtn')) return;
    const btn = document.createElement('button');
    btn.id = 'globalAdminLogoutBtn';
    btn.textContent = 'Uitloggen';
    btn.style.cssText = 'position:fixed;top:14px;right:14px;z-index:10001;background:#111;color:#fff;border:1px solid rgba(212,175,55,.35);border-radius:14px;padding:10px 14px;font:700 14px/1 Inter,system-ui,sans-serif;box-shadow:0 10px 24px rgba(0,0,0,.18);cursor:pointer;';
    btn.onclick = ()=>{ clearAll(); window.location.href='./admin.html'; };
    document.body.appendChild(btn);
  }

  function redirectToAdmin(reason='device_required'){
    if (window.GEJAST_ADMIN_SESSION && typeof window.GEJAST_ADMIN_SESSION.redirectToAdminLogin === 'function') {
      return window.GEJAST_ADMIN_SESSION.redirectToAdminLogin(reason);
    }
    const raw = (window.location.pathname.split('/').pop() + window.location.search + window.location.hash);
    const here = encodeURIComponent(safeReturnTarget(raw,'admin.html'));
    window.location.href = `./admin.html?reason=${encodeURIComponent(reason)}&return_to=${here}`;
  }
  ...
  async function gate(){
    ...
    if (!hasUsableLocalSession()) {
      clearAll();
      redirectToAdmin('session_required');
      return;
    }
    document.documentElement.classList.remove('admin-gate-pending');
    addLogout();
    try {
      if (window.GEJAST_ADMIN_SESSION && typeof window.GEJAST_ADMIN_SESSION.backgroundValidate === 'function') {
        const data = await window.GEJAST_ADMIN_SESSION.backgroundValidate();
        if (data) setSession(data?.admin_session_token || t, data?.admin_username || u, data?.raw_device_token || d);
        return;
      }
      ...
      console.warn('Admin gate background validation failed', err);
      if (!hasUsableLocalSession()) {
        clearAll();

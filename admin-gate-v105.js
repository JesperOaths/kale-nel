(function(){
  ...
  function redirectToAdmin(reason='device_required'){ if(window.GEJAST_ADMIN_SESSION && typeof window.GEJAST_ADMIN_SESSION.redirectToAdminLogin === 'function'){ return window.GEJAST_ADMIN_SESSION.redirectToAdminLogin(reason); } const raw=(window.location.pathname.split('/').pop() + window.location.search + window.location.hash); const here=encodeURIComponent(safeReturnTarget(raw,'admin.html')); window.location.href=`./admin.html?reason=${encodeURIComponent(reason)}&return_to=${here}`; }
  function hasUsableLocalSession(){
    const t=token();
    const until=deadline();
    return !!(t && (!until || Date.now() < until));
  }
  async function gate(){
    const t=token(), d=device(), u=username();
    if(!hasUsableLocalSession()){ clearAll(); redirectToAdmin('session_required'); return; }

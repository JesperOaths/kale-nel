...
for(const file of contrastFiles){const text=fs.readFileSync(file,'utf8');if(text.includes('#8a7a55')) failures.push(file+' reintroduced the audited low-contrast #8a7a55 token');if(!text.includes('#827148')) failures.push(file+' lost the audited AA-safe #827148 replacement token');}
...
for(const path of ['V785_DIAGNOSTIC_RESULTS.json','V785_DIAGNOSTIC_SUMMARY.txt','scripts/diagnose-v785-final-findings.mjs','scripts/summarize-v785-diagnostic.mjs','scripts/build-v785-final-acceptance-fixes.mjs','scripts/prove-v785-final-fixes.mjs','.github/workflows/v785-targeted-diagnostic.yml','.github/workflows/v785-release-builder.yml']) if(fs.existsSync(path)) failures.push('temporary v785 audit/builder residue remains: '+path);
...
console.log('v785 final acceptance regression PASS: audited contrast owners are AA-safe, standalone Klaverjas fields are labelled, Rad mobile containment is retained, and temporary audit/builder residue is absent.');

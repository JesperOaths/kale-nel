-- GEJAST v722 RPC contract repair SQL
-- Fixes:
...
create or replace function public._paardenrace_next_despinoza_room_code_v722()
returns text
language plpgsql
security definer
...
begin
  ...
  loop
    ...
    v_code := public._paardenrace_next_despinoza_room_code_v722();
    begin
...
set search_path to 'public'
as $fn$
  select public.join_paardenrace_room_safe(session_token, session_token_input, room_code_input)
$fn$;
...
set search_path to 'public'
as $fn$
  select public.reset_paardenrace_room_safe(session_token, session_token_input, room_code_input)
$fn$;
...
create or replace function public._pikken_state_public_v722(
...
set search_path to 'public'
as $fn$
  select public._pikken_state_public_v722(game_id_input, null, lobby_code_input, session_token, session_token_input, site_scope_input)
$fn$;
...
set search_path to 'public'
as $fn$
  select public._pikken_state_public_v722(game_id_input, client_match_id, lobby_code_input, session_token, session_token_input, site_scope_input)
$fn$;
...
set search_path to 'public'
as $fn$
  select public._pikken_state_public_v722(game_id_input, null, lobby_code_input, null, null, site_scope_input)
$fn$;

grant execute on function public._paardenrace_next_despinoza_room_code_v722() to anon, authenticated;
...
grant execute on function public._pikken_state_public_v722(uuid,uuid,text,text,text,text) to anon, authenticated;

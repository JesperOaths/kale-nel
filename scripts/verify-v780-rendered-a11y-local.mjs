...
await context.route('**/*',async route=>{
  ...
  if(/\/gejast-config\.js$/i.test(url.pathname) && ['127.0.0.1','localhost'].includes(url.hostname)){
    ...
    return route.fulfill({response:upstream,contentType:'application/javascript',body:`${body}\n;window.GEJAST_CONFIG=window.GEJAST_CONFIG||{};window.GEJAST_CONFIG.requireMatchEntrySession=function(){return true;};`});
  }
...
assert.ok(configOverrides>0,'local audit expected to override match-entry redirect in local gejast-config.js');
...
console.log(`V780_LOCAL_RENDERED_A11Y=PASS routes=${routes.length} gateStubs=${gateStubs} configOverrides=${configOverrides} blockedNonGet=${blockedWrites}`);

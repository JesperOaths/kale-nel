...
async function audit(path,label,width,height){
  ...
  let navigationError='';
  try{await page.goto(`${base}${path}?final_acceptance=${Date.now()}`,{waitUntil:'domcontentloaded',timeout:30000});}catch(err){navigationError=clean(err?.message||err);}
  ...
  const state=await page.evaluate(()=>{
    ...
    const positiveTabindex=focusables.filter(el=>Number(el.getAttribute('tabindex'))>0).length;
    return{
      ...
      positiveTabindex,hiddenFocusable,
  ...
  if(!navigationError&&state.bodyText){
    try{await page.addScriptTag({content:axeSource});const out=await page.evaluate(async()=>await window.axe.run(document,{runOnly:{type:'tag',values:['wcag2a','wcag2aa','wcag21a','wcag21aa']},resultTypes:['violations']}));axe=out.violations.filter(v=>['serious','critical'].includes(v.impact)).map(v=>({id:v.id,impact:v.impact,nodes:v.nodes.length,help:v.help}));}catch(err){axe=[{id:'axe-run-failed',impact:'critical',nodes:1,help:clean(err?.message||err)}];}
  }
  ...
  rows.push(row);console.log(`FINAL ${label} ${path} ${JSON.stringify(row)}`);await context.close();
}
...
const summary={combinations:rows.length,failures:failures.length,blockedWrites,details:failures.map(r=>({path:r.path,viewport:r.viewport,navigationError:r.navigationError,pageErrors:r.pageErrors,consoleErrors:r.consoleErrors,badSameOrigin:r.badSameOrigin,failedSameOrigin:r.failedSameOrigin,state:r.state,axe:r.axe}))};
console.log('FINAL_ACCEPTANCE_SUMMARY='+JSON.stringify(summary));
if(blockedWrites===0)throw new Error('final browser audit did not demonstrate write interception');
if(failures.length){console.error(`Final browser acceptance found ${failures.length} failing combinations.`);process.exit(1);}
console.log(`FINAL_BROWSER_ACCEPTANCE=PASS combinations=${rows.length} blockedWrites=${blockedWrites} seriousCriticalAxe=0`);

...
for(const width of widths){
 ...
 await context.route('**/*',async route=>{const req=route.request();if(!['GET','HEAD'].includes(req.method())){blockedWrites++;return route.fulfill({status:200,contentType:'application/json',body:'[]'});}return route.continue();});
 page.on('pageerror',e=>pageErrors.push(String(e?.message||e)));
 page.on('response',res=>{const u=new URL(res.url());if(u.hostname==='127.0.0.1'&&['GET','HEAD'].includes(res.request().method())&&res.status()>=400)bad.push(`${res.status()} ${u.pathname}`);});
 page.on('requestfailed',req=>{try{const u=new URL(req.url());if(u.hostname==='127.0.0.1'&&['GET','HEAD'].includes(req.method()))failed.push(`${u.pathname}: ${req.failure()?.errorText||'failed'}`);}catch{}});
 ...
 if(nav||bad.length||failed.length||pageErrors.length||axe.length||overflow||wheelOutside||runtimeWrong) failures.push({width,nav,bad,failed,pageErrors,axe,state,overflow,wheelOutside,runtimeWrong});
...
await browser.close();
if(blockedWrites===0) failures.push({width:'safety',reason:'non-GET interception not exercised'});

...
function walk(rel) {
  ...
  if (st.isFile()) { files.push(posix(rel)); return; }
...
function report(file, text, index, message) { diagnostics.push(`${file}:${lineOf(text, index)}: ${message}`); }
function warn(file, text, index, message) { warnings.push(`${file}:${lineOf(text, index)}: ${message}`); }
function stripUrl(raw) { return raw.trim().replace(/^['"]|['"]$/g, '').split('#')[0].split('?')[0]; }
...
function resolveLocal(fromFile, raw) {
  ...
  const rel = clean.startsWith('/') ? clean.replace(/^\/+/, '') : posix(path.join(path.dirname(fromFile), clean));
...
function candidatePaths(rel) {
  ...
  if (!path.posix.extname(rel)) { out.push(`${rel}.html`); out.push(`${rel}/index.html`); }
  return [...new Set(out)];
}
...
function markupOnly(text) {
  // Preserve length/newlines so diagnostics still point at original source lines,
  // but exclude JavaScript/CSS source where template strings can contain id/href/src markup
  // that is not simultaneously present in the document DOM.
  return text
    .replace(/<script\b[^>]*>[\s\S]*?<\/script\s*>/gi, (block) => block.replace(/[^\n]/g, ' '))
    .replace(/<style\b[^>]*>[\s\S]*?<\/style\s*>/gi, (block) => block.replace(/[^\n]/g, ' '));
}
...
for (const file of htmlFiles) {
  ...
  const ids = new Map();
  for (const match of markup.matchAll(/\bid\s*=\s*(["'])([^"']+)\1/gi)) {
    const id = match[2];
    if (!ids.has(id)) ids.set(id, []);
    ids.get(id).push(match.index ?? 0);
  }
  for (const [id, positions] of ids) {
    if (positions.length > 1) report(file, text, positions[1], `duplicate static id=${JSON.stringify(id)} (${positions.length} occurrences)`);
  }
  ...
  for (const match of markup.matchAll(attrRe)) {
    ...
    if (!rel) continue;
    if (!candidatePaths(rel).some((p) => fs.existsSync(path.join(ROOT, p)))) {
      report(file, text, match.index ?? 0, `${tag}[${attr}] references missing local target ${JSON.stringify(raw)} -> ${rel}`);
    }
    if (rel.split('/').map((part) => part.toLowerCase()).some((part) => historicalParts.has(part))) {
      report(file, text, match.index ?? 0, `${tag}[${attr}] points into historical mirror path ${JSON.stringify(raw)}`);
    }
  }

  for (const match of markup.matchAll(/\b(?:href|src)\s*=\s*(["'])(http:\/\/[^"']+)\1/gi)) {
    const url = match[2];
  ...
  for (const match of markup.matchAll(/<a\b[^>]*\btarget\s*=\s*(["'])_blank\1[^>]*>/gi)) {
    if (!/\brel\s*=\s*(["'])[^"']*\bnoopener\b[^"']*\1/i.test(match[0])) {

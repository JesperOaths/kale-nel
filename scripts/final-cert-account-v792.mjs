...
try {
  ...
  if (MODE === 'activate_login') {
    ...
    // The page redirects shortly after success. Storage survives navigation.
    ...
    // Client logout must remove the browser token.
    ...
    // A truly fresh browser context proves the PIN/login path, not residual storage.

...
function auditInit(){
  ...
  const bm=String(qs.get('auditbid')||'').match(/^(\d+)x([1-6])$/);
  let persisted=[];
  try{persisted=JSON.parse(sessionStorage.getItem('__v791_audit_calls')||'[]');if(!Array.isArray(persisted))persisted=[];}catch(_){persisted=[];}
  ...
  const record=(name,payload={})=>{
    ...
    try{sessionStorage.setItem('__v791_audit_calls',JSON.stringify(audit.calls));}catch(_){}
  };
  try{
  ...
  window.confirm=()=>true;
  window.alert=(message)=>record('ALERT',{message:String(message||'')});
  ...
  const mock=(name,payload)=>{
    ...
    if(name==='pikken_cast_vote_scoped'){record('VOTE_VALUE',{vote:payload?.vote_input});p.stateVersion++;return pikkenState();}
  ...
  window.fetch=async function(input,init={}){
    ...
    if(url.hostname.includes('supabase.co')){
      record(`UNEXPECTED_BACKEND:${url.pathname}`,{});
...
async function testPaardenrace(context){
  ...
  await page.locator('#suitInput').selectOption('spades');await page.locator('#wagerInput').fill('1');await page.locator('#saveBtn').click();
  ...
  const render=await page.evaluate(()=>{
    const api=window.GEJAST_PAARDENRACE,red=api.parseCard('AH'),black=api.parseCard('AS');
  ...
  await page.locator('#startBtn').click({noWaitAfter:true});
  await page.waitForURL(/paardenrace_live\.html/, {timeout:7000});
  assert.equal(await hasCall(page,'start_paardenrace_countdown_safe'),true,'Paardenrace start RPC missing');
  assert.ok(new URL(page.url()).pathname.endsWith('/paardenrace_live.html'),'Paardenrace did not finish on live page');
  await noOverflow(page,'paardenrace-live');await noErrors(errors,'paardenrace');await page.close();
}
...
async function testPikkenLobby(context){
  ...
  await page.locator('#pkReadyBtn').waitFor({state:'visible',timeout:5000});await page.locator('#pkReadyBtn').click();
  ...
  await page.locator('#pkStartBtn').click({noWaitAfter:true});
  ...
  assert.ok(new URL(page.url()).pathname.endsWith('/pikken_live.html'),'Pikken did not finish on live page');
  await noOverflow(page,'pikken-live-after-start');await noErrors(errors,'pikken-lobby');await page.close();
}
...
async function testPikkenBidRule(context){
  ...
  await high.page.locator('#bidSelect').selectOption('2:1');await high.page.locator('#bidBtn').click();await high.page.waitForTimeout(250);
  assert.equal(await hasCall(high.page,'pikken_place_bid_scoped'),true,'Pikken live bid RPC missing');
  const dice=await high.page.locator('#diceStateNote img.die').count();
  assert.equal(dice,6,`Pikken private hand should render exactly six dice (${dice})`);
...
async function runCase(engineName,browser,viewName,viewport,label,fn){
  ...
  try{
    ...
    passes.push(`${engineName}:${viewName}:${label}`);console.log(`V791_ACCEPT_PASS ${engineName} ${viewName} ${label}`);
  }catch(e){failures.push(`${engineName}:${viewName}:${label}: ${e?.stack||e}`);console.error(`V791_ACCEPT_FAIL ${engineName} ${viewName} ${label}: ${e?.stack||e}`);}
...
console.log(`V791_CORE_ACCEPT_PASSES=${passes.length}`);console.log(`V791_CORE_ACCEPT_FAILURES=${failures.length}`);
if(failures.length){console.error('V791_CORE_GAMEPLAY_ACCEPTANCE=FAIL');failures.forEach(f=>console.error(`- ${f}`));process.exit(1);}

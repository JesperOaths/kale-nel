  ...
  return {
    ...
    ASSETS: {
      async fetch(request) {
        ...
        if (url.pathname === '/admin.html') return new Response('<!doctype html><title>Beheerhub - Wordt er gejast?</title><script>window.GEJAST_PAGE_VERSION=\'v762\';</script><script src="./gejast-home-gate.js?v771"></script><script src="./admin-session-sync.js?v771"></script>', { status: 200, headers: { 'Content-Type': 'text/html' } });
...
assert.match(approvedHtml, /GEJAST_PAGE_VERSION='v771'/);
assert.match(approvedHtml, /gejast-home-gate\.js\?v771/);
assert.match(approvedHtml, /admin-session-sync\.js\?v771/);
...
const publicAsset = await req('https://kalenel.nl/gejast-admin-rpc.js?v771', { redirect: 'manual' });
assert.equal(publicAsset.status, 302);
assert.equal(publicAsset.headers.get('Location'), 'https://admin.kalenel.nl/gejast-admin-rpc.js?v771');
...
try {
  ...
  assert.match(adminAfterCallbackHtml, /GEJAST_PAGE_VERSION='v771'/);
} finally {

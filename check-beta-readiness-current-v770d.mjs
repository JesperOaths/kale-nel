...
for (const id of [
  'real_device_push_delivery','toepen_backend_live','admin_host_security','scope_isolation','analytics_observability',
...
const toepenProof = [toepenGap?.proof_needed, toepenGap?.next_action, toepenGap?.latest_probe].map((v) => String(v || '')).join('\n');
if (!/v801a/i.test(toepenProof)) failures.push('completed Toepen evidence must name v801a');
if (!/20260818001412/.test(toepenProof)) failures.push('completed Toepen evidence must preserve production migration 20260818001412');
if (!/32084142660/.test(toepenProof)) failures.push('completed Toepen evidence must preserve the public REST owner-guard proof run');
if (!/same-owner/i.test(toepenProof) || !/cross-owner/i.test(toepenProof)) failures.push('completed Toepen evidence must preserve same-owner/cross-owner outcomes');
if (/needs_permission|authorization before production deployment|not deployed/i.test(toepenProof)) failures.push('completed Toepen evidence must not regress to a pre-deployment permission blocker');

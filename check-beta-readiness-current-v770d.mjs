...
if (!String(tracker.site_version || '').includes(version)) {
  failures.push(`tracker site_version must mention ${version}`);
}
if (!/^\d{4}-\d{2}-\d{2}$/.test(String(tracker.last_updated || ''))) {
  failures.push('tracker last_updated must be YYYY-MM-DD');
}
...
for (const stale of [
  ...
  'Backend user-targeted delivery still needs',
  'fix public admin-host exposure separately',
]) {
...
for (const id of ['real_device_push_delivery', 'admin_host_security', 'scope_isolation', 'analytics_observability']) {
...
if (completeCount !== 7 || permissionCount !== 5) {
  failures.push(`expected current readiness split 7 complete / 5 permission-gated, got ${completeCount} / ${permissionCount}`);
}

if (!extended.includes("const adminBaseUrl = process.env.GEJAST_ADMIN_BASE_URL || 'https://admin.kalenel.nl';")) {
  failures.push('extended beta checker must target the protected admin host');
}
if (!extended.includes('if (response.status !== 401)')) {
  failures.push('extended beta checker must require unauthenticated admin HTTP 401');
}
if (!extended.includes("{ area: 'adminObservability', path: '/admin_system_health.html', protected: true }")) {
  failures.push('extended beta checker must mark admin observability routes protected');
}

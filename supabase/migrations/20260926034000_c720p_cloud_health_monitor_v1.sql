...
begin
  ...
  perform c720p_security.set_health_alert_v1(
    ...
    case when found then 'C720P compact health heartbeat is stale.'
         else 'C720P compact health heartbeat has not reported yet.' end,
  ...
  if not stale then
    ...
    perform c720p_security.set_health_alert_v1(
      'drive_upload_quota',drive_status in ('quota_exceeded','quota_backoff'),'high',
      case when drive_retry<>'' then
        'C720P camera archive uploads are blocked by Google Drive storage quota until the next retry window ('||drive_retry||').'
      else 'C720P camera archive uploads are blocked by Google Drive storage quota.' end,
      coalesce(p->'drive','{}'::jsonb)
    );
    perform c720p_security.set_health_alert_v1(
      'drive_quota_exceeded',false,'high','Legacy Drive quota alert key.','{}'::jsonb
    );
    ...
    perform c720p_security.set_health_alert_v1(
      'gmail_triage_degraded',false,'medium','Legacy Gmail triage alert key.','{}'::jsonb
    );
    ...
    perform c720p_security.set_health_alert_v1(
      ...
      case when cardinality(voice_bad)>0 then
        'C720P voice stack is missing listener(s): '||array_to_string(voice_bad,', ')||'.'
    ...
    exception when others then backup_age := null; end;
    perform c720p_security.set_health_alert_v1(
      'c720p_backup_stale',backup_age is null or backup_age > 172800,'medium',
      case when backup_age is null then 'C720P backup age is unavailable.'
           else 'Latest C720P recovery backup is older than 48 hours.' end,
      coalesce(p->'backup','{}'::jsonb)
    );

    foreach k in array array[
      'c720p-agent-runner.service',
      'c720p-frontyard-security.service',
...
$$;
revoke all on function c720p_security.evaluate_push_health_v2() from public,anon,authenticated;
grant execute on function c720p_security.evaluate_push_health_v2() to service_role;

do $$
begin
  if exists(select 1 from cron.job where jobname='c720p_health_push_evaluator_v2') then
    perform cron.unschedule('c720p_health_push_evaluator_v2');
  end if;
end $$;
select cron.schedule(
  'c720p_health_push_evaluator_v2','*/5 * * * *',

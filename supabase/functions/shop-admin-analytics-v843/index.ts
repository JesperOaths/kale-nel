...
import { buildGrowthIntelligence, recordPriceCostHistory } from "../_shared/shop-admin-growth-v843.mjs";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === "campaign_spend_add") {
      const occurred_on=text(body?.occurred_on),amount=Number(body?.amount_cents);
      if(!/^\d{4}-\d{2}-\d{2}$/.test(occurred_on)) return json(req,{error:"invalid_campaign_date"},400);
      if(!Number.isInteger(amount)||amount<=0||amount>100000000) return json(req,{error:"invalid_campaign_amount"},400);
      const row={occurred_on,utm_source:text(body?.utm_source).slice(0,120),utm_medium:text(body?.utm_medium).slice(0,120),utm_campaign:text(body?.utm_campaign).slice(0,180),amount_cents:amount,note:text(body?.note).slice(0,1000)||null,created_by_admin_id:admin.admin_id,updated_at:new Date().toISOString()};
      const {data,error}=await sb.from("shop_campaign_spend_v843").insert(row).select().single(); if(error)throw error;
      await auditAdminAction(sb,admin,"campaign_spend_add",{object_type:"campaign_spend",object_id:String(data?.id||""),metadata:{occurred_on,amount_cents:amount,utm_source:row.utm_source,utm_medium:row.utm_medium,utm_campaign:row.utm_campaign}});
      return json(req,{ok:true,entry:data});
    }
    if (action === "campaign_spend_delete") {
      const id=Number(body?.id); if(!Number.isInteger(id)||id<=0)return json(req,{error:"invalid_campaign_spend_id"},400);
      const {data,error}=await sb.from("shop_campaign_spend_v843").delete().eq("id",id).select("id").maybeSingle();if(error)throw error;
      await auditAdminAction(sb,admin,"campaign_spend_delete",{object_type:"campaign_spend",object_id:String(id),metadata:{deleted:!!data}});
      return json(req,{ok:true,deleted:!!data});
    }
    if (action === "goal_upsert") {
      const period_month=text(body?.period_month),metric=text(body?.metric),target=Number(body?.target_value);
      const allowed=new Set(["recognized_sales_cents","known_contribution_cents","paid_orders","conversion_rate","repeat_customer_rate"]);
      if(!/^\d{4}-\d{2}-01$/.test(period_month))return json(req,{error:"invalid_goal_month"},400);
      if(!allowed.has(metric))return json(req,{error:"invalid_goal_metric"},400);
      if(!Number.isFinite(target)||target<0||target>1000000000)return json(req,{error:"invalid_goal_target"},400);
      const row={period_month,metric,target_value:target,note:text(body?.note).slice(0,1000)||null,created_by_admin_id:admin.admin_id,updated_at:new Date().toISOString()};
      const {data,error}=await sb.from("shop_goals_v843").upsert(row,{onConflict:"period_month,metric"}).select().single();if(error)throw error;
      await auditAdminAction(sb,admin,"goal_upsert",{object_type:"shop_goal",object_id:String(data?.id||""),metadata:{period_month,metric,target_value:target}});
      return json(req,{ok:true,entry:data});
    }
    if (action === "goal_delete") {
      const id=Number(body?.id);if(!Number.isInteger(id)||id<=0)return json(req,{error:"invalid_goal_id"},400);
      const {data,error}=await sb.from("shop_goals_v843").delete().eq("id",id).select("id").maybeSingle();if(error)throw error;
      await auditAdminAction(sb,admin,"goal_delete",{object_type:"shop_goal",object_id:String(id),metadata:{deleted:!!data}});
      return json(req,{ok:true,deleted:!!data});
    }
    if (action === "annotation_add") {
      const occurred_at=isoOrNull(body?.occurred_at),category=text(body?.category)||"note",title=text(body?.title).slice(0,160),note=text(body?.note).slice(0,2000);
      const allowed=new Set(["note","campaign","price_change","product_change","operations","other"]);
      if(!occurred_at)return json(req,{error:"invalid_annotation_time"},400);
      if(!allowed.has(category))return json(req,{error:"invalid_annotation_category"},400);
      if(!title)return json(req,{error:"annotation_title_required"},400);
      const row={occurred_at,category,title,note:note||null,created_by_admin_id:admin.admin_id,updated_at:new Date().toISOString()};
      const {data,error}=await sb.from("shop_annotations_v843").insert(row).select().single();if(error)throw error;
      await auditAdminAction(sb,admin,"annotation_add",{object_type:"annotation",object_id:String(data?.id||""),metadata:{occurred_at,category,title}});
      return json(req,{ok:true,entry:data});
    }
    if (action === "annotation_delete") {
      const id=Number(body?.id);if(!Number.isInteger(id)||id<=0)return json(req,{error:"invalid_annotation_id"},400);
      const {data,error}=await sb.from("shop_annotations_v843").delete().eq("id",id).select("id").maybeSingle();if(error)throw error;
      await auditAdminAction(sb,admin,"annotation_delete",{object_type:"annotation",object_id:String(id),metadata:{deleted:!!data}});
      return json(req,{ok:true,deleted:!!data});
    }
    ...
    console.error("shop-admin-analytics-v843 failed", invalid ? "invalid_admin_session" : message.slice(0,300));

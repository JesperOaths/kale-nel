...
begin
  ...
  loop
    insert into public.toepen_rounds(
      ...
      greatest(1,coalesce(nullif(round_row->>'stake_final','')::integer,1)),
    ...
    loop
      insert into public.toepen_round_results(
        ...
        nullif(result_row->>'seat_no','')::integer,
        coalesce(nullif(trim(result_row->>'name'),''),'Onbekend'),
        case when result_row->>'action' in ('win','stay','fold') then result_row->>'action' else 'stay' end,
        greatest(0,coalesce(nullif(result_row->>'penalty_points','')::integer,0)),
        nullif(result_row->>'folded_at_stake','')::integer
      );
    end loop;
  end loop;
...
begin
  ...
  if to_regprocedure('public.admin_check_session(text)') is not null then
    begin
      ...
      if state is not null then return true; end if;
    exception when others then
      return false;
    end;
  end if;

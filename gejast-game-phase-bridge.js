(function(){
  ...
  const VERSION='v658';
  const RPC={
    audit:'admin_get_game_group_a_audit_v658',
    generic:'get_game_group_a_bundle_v658',
    beerpong:'get_beerpong_phase_bundle_v658',
    boerenbridge:'get_boerenbridge_phase_bundle_v658'
  };
  ...
  function renderPanel(data, target){
    ...
    if(!data||data.ok===false){body.innerHTML=`<div>${escapeHtml(data&&data.error||'Geen data beschikbaar. Run de v658 SQL in Supabase.')}</div>`;return;}
    const totals=data.totals||{};
    const leaders=Array.isArray(data.leaderboard)?data.leaderboard:[];
    body.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:8px;margin-bottom:10px"><div><b>${escapeHtml(totals.players||0)}</b><br><span>spelers</span></div><div><b>${escapeHtml(totals.matches||0)}</b><br><span>matches</span></div><div><b>${escapeHtml(totals.avg_elo||0)}</b><br><span>gem. ELO</span></div><div><b>${escapeHtml(data.source||'v658')}</b><br><span>bron</span></div></div>`+

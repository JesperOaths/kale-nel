-- GEJAST v701: correct Pikken reject/vote/reveal rules and direct Paardenrace ready.

begin;

drop function if exists public._gejast_scope_norm_v701(text);

create or replace function public._gejast_scope_norm_v701(site_scope_input text default 'friends')
returns text
language sql
stable
as $fn$
  ...
  for rec in
    ...
    where n.nspname = 'public'
      and p.proname in (
        ...
        '_pikken_next_alive_seat_v701',
        '_pikken_deal_round_v701',
        '_pikken_finish_vote_v701',
...
begin
  ...
  if not found then
    raise exception 'Je zit niet in deze Paardenrace room.';
  end if;
...
create or replace function public._pikken_next_alive_seat_v701(
  game_id_input uuid,
...
create or replace function public._pikken_deal_round_v701(
  game_id_input uuid,
  round_no_input integer
)
returns void
language plpgsql
security definer
...
begin
  ...
  where id = game_id_input
    and public._gejast_scope_norm_v701(site_scope)=public._gejast_scope_norm_v701(site_scope_input)
  for update;
  ...
  values (g.id, v_round, p.id, false);
  ...
  update public.pikken_games
     set status = 'voting',
         state = coalesce(state,'{}'::jsonb)
           || jsonb_build_object(
             'phase','voting',
             'vote_turn_seat',public._pikken_next_alive_seat_v701(g.id, gp.seat_index, v_bidder_id),
             'challenger_id',p.id,
  ...
  return public._pikken_finish_vote_v701(g.id, coalesce(session_token_input, session_token), site_scope_input);
end
$fn$;

create or replace function public._pikken_finish_vote_v701(
  game_id_input uuid,
...
begin
  ...
  where game_id = g.id
    and eliminated_at is null
    and player_id is distinct from v_bidder_id;
  ...
  if v_votes < greatest(1, v_eligible) then
    update public.pikken_games
       set state = coalesce(state,'{}'::jsonb)
             || jsonb_build_object('vote_turn_seat', public._pikken_next_alive_seat_v701(g.id, coalesce((g.state->>'vote_turn_seat')::integer,0), v_bidder_id)),
           state_version = coalesce(state_version,0)+1,
           updated_at = now()
     where id = g.id;
  ...
  v_loser_id := case
    when v_mode in ('right_loses','goed_verliest','fair') then
      case when v_bid_true then v_bidder_id else v_challenger_id end
    else
      case when v_bid_true then v_challenger_id else v_bidder_id end
  end;

  select coalesce(jsonb_agg(jsonb_build_object('seat', gp.seat_index, 'name', gp.player_name, 'dice', h.dice_values) order by gp.seat_index), '[]'::jsonb)
    into v_hands
  from public.pikken_round_hands h
    ...
    update public.pikken_games
       set status = 'finished',
           ...
           state = coalesce(state,'{}'::jsonb)
             || jsonb_build_object(
               'phase','finished',
               'last_reveal',jsonb_build_object('round_no',v_round,'bid',v_bid,'bid_true',v_bid_true,'counted_total',v_total,'loser_id',v_loser_id,'hands',v_hands)
             ),
           state_version = coalesce(state_version,0)+1,
           updated_at = now()
     where id = g.id;
  ...
  v_next_seat := public._pikken_next_alive_seat_v701(g.id, coalesce((select seat_index from public.pikken_game_players where game_id = g.id and player_id = v_loser_id), 0), null);
  perform public._pikken_deal_round_v701(g.id, v_next_round);

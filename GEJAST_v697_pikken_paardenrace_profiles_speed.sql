-- GEJAST v697: Pikken bid-hit helper, dice totals, Paardenrace conflict fix, Paardenrace Bakken drink verification, Beurs wallet default.
...
create or replace function public._pikken_count_bid_hits(
  dice_values integer[],
  bid_face integer
)
returns integer
...
as $fn$
  with dice as (
    select v
    from unnest(coalesce(dice_values, array[]::integer[])) as d(v)
    where v between 1 and 6
  ),
  facts as (
    select
      count(*) filter (where v = 1) as piks,
      count(*) filter (where v = bid_face) as exact_hits,
      count(distinct v) as distinct_faces,
      count(*) as total_dice
    from dice
  )
  select case
    when bid_face = 1 and coalesce(distinct_faces, 0) = 6 and coalesce(total_dice, 0) >= 6 then 6
    when bid_face = 1 then coalesce(piks, 0)::integer
    when bid_face between 2 and 6 then (coalesce(exact_hits, 0) + coalesce(piks, 0))::integer
    else 0
  end
  from facts
$fn$;
...
immutable
as $fn$
  select public._pikken_count_bid_hits(dice_values, bid_face::integer)
$fn$;
...
grant execute on function public._pikken_count_bid_hits(integer[],numeric) to anon, authenticated;
